...
static int disableCoreDumps(void);
...
/* mitigateForensicTraces disables core dump and swapping in an attempt to 
 * prevent the application from leaving unexpected forensic traces on the 
 * hard drive.
 *
...
{
  /* Disable core dumps to prevent forensic traces */
  if( !disableCoreDumps() ){
    logErr("Failed to disable core dumps to prevent forensic traces");
    return 0;
  }
  
  /* Prevent using swap to prevent forensic traces */ 
  if( mlockall(MCL_ONFAULT | MCL_FUTURE | MCL_CURRENT) ){
    logErr("Failed to disable swap to prevent forensic traces");
    return 0;
  }
