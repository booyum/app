...
//note ptrace can disable seccomp disable it in seccomp 
//in /proc/<pid>/cwd block off
//need pid namespaces
...
{ 
  ...
  if( initNs != INIT_FSNS && initNs != NO_INIT_FSNS ){
    printf("Invalid argument passed to isolFs");
    return 0; 
  } 
   ...
   */ 
  if( mount(path, path, "ext4", MS_BIND , NULL) ){ 
    printf("sandbox directory appears to be missing");
    return 0; 
  }
  
    if( mount("/dev", "sandbox/dev", "devtmpfs", MS_NOSUID , NULL) ){ 
    printf("sandbox directory appears to be missing");
    return 0; 
  }
  
  if( mount("/proc", "sandbox/proc", "proc", MS_NOSUID , NULL) ){ 
    printf("sandbox directory appears to be missing");
    return 0; 
  }
  ...
  */
  ...
  /* Switch the process's current directory to be root, which is actually the
