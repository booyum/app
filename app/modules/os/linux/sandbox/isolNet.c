...
static int  initUnixListen(char *path, int pathBc);
...
static int getTorSock(void)
{
 ...
 /* initgTors initializes the static global struct sockaddr and socklen_t used
  * for making connections to the Tor SocksPort. Note that SECCOMP will whitelist
  * only these initialized variables for use with the connect() syscall for this
  * process. The memory backing of the struct is a memory pane that is set to 
 ...
 *
 * The memory pointed to by gTorAddr is mprotected to real only after 
...
/* initUnixListen returns a bound and listening Unix Domain Socket on the path 
 * pointed to by 'path', which is of pathBc bytes, or otherwise -1 on error. 
 */ 
static int initUnixListen(char *path, int pathBc)
{
  ...
  if( path == NULL || pathBc == 0 ){
    logErr("Something was NULL that shouldn't have been");
    return -1; 
  }
  
  if( pathBc > 107 ){
    logErr("Path is too large to be used as a Unix Domain Socket");
    return -1; 
  }
  
  /* Get the Unix domain socket */
  unixListen = socket(AF_UNIX, SOCK_STREAM, 0);
  if( unixListen == -1 ){
  ...
  secStrCpy(local.sun_path, path, 108); /* NOTE: See beej for 108 figure */ 
  ...
  if( bind(unixListen, (struct sockaddr *)&local, len) || 
      listen(unixListen, 20) ){ 
    return -1;
  }
  
  /* All went well, return the bound and listening unix domain socket */
  return unixListen; 
}
